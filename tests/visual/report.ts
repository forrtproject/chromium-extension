...
type Result = {name: string; status: string; detail?: string; changed?: boolean; baseFailed?: boolean};
...
<pre>${escape(err instanceof Error ? err.message : String(err))}</pre>`);
  ...
  const added = results.filter(r => isNew(r) && !r.baseFailed);
  // Existing fixtures whose base capture failed: nothing to compare against.
  const baseFailed = results.filter(r => isNew(r) && r.baseFailed);
...
<p>Captured ${results.length} fixture${results.length === 1 ? "" : "s"}: ${changed.length} changed, ${added.length} new, ${baseFailed.length} without a base image, ${unchanged.length} unchanged.</p>
...
${baseFailed.length ? `<h2>Base capture failed</h2>${baseFailed.map(r => row(r, ["actual"])).join("")}` : ""}
